...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" dir="{{ $isRtl ? 'rtl' : 'ltr' }}">
    <head>
        ...
        <title>{{ config('app.name', 'RestoSmart') }}</title>
    ...
    <body class="font-sans antialiased">
        <div class="min-h-screen bg-zinc-50 text-zinc-950 dark:bg-zinc-950 dark:text-zinc-50" x-data="{ sidebarOpen: false }">
            @auth
                    ...
                    <header class="sticky top-0 z-20 border-b border-zinc-200 bg-zinc-50/90 backdrop-blur dark:border-zinc-800 dark:bg-zinc-950/90 lg:hidden">
                        <div class="flex h-16 items-center justify-between px-4">
                            ...
                            <a href="{{ $tenantId ? route('tenant.menu', $tenantId) : route('home') }}" class="flex items-center gap-2 font-semibold">
                                <x-application-logo class="h-8 w-8 text-red-700 dark:text-red-400" />
                ...
                <header class="border-b border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
                    <div class="mx-auto flex h-16 max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <x-theme-switcher />
                            <a href="{{ $tenantId ? route('tenant.login', $tenantId) : route('login') }}" class="hidden items-center gap-2 rounded-lg bg-red-700 px-4 py-2 text-sm font-semibold text-white hover:bg-red-800 app-focus sm:inline-flex">
