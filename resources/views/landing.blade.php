...
<x-app-layout>
    <section class="relative isolate overflow-hidden border-b border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
        ...
        <div class="relative mx-auto flex min-h-[68svh] max-w-7xl items-center px-4 py-16 sm:px-6 lg:px-8">
            <div class="max-w-3xl">
                <p class="inline-flex items-center gap-2 text-sm font-semibold uppercase text-red-700 dark:text-red-300">
                ...
                <div class="mt-8 flex flex-col gap-3 sm:flex-row">
                    <a href="{{ route('restaurants.apply') }}" class="inline-flex items-center justify-center gap-2 rounded-lg bg-red-700 px-5 py-3 text-sm font-semibold text-white shadow-sm shadow-red-700/20 transition hover:bg-red-800 app-focus">
    ...
    <section class="bg-zinc-50 py-16 dark:bg-zinc-950">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="max-w-2xl">
                <p class="text-sm font-semibold uppercase text-red-700 dark:text-red-300">{{ __('Built for restaurant service') }}</p>
            ...
            <div class="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                @foreach ($featureCards as $feature)
                    <article class="rounded-lg border border-zinc-200 bg-white p-5 transition hover:border-red-300 hover:bg-red-50/40 dark:border-zinc-800 dark:bg-zinc-900/80 dark:hover:border-red-900 dark:hover:bg-red-950/20">
                        <span class="grid h-10 w-10 place-items-center rounded-lg border border-red-200 bg-red-50 text-red-700 dark:border-red-900 dark:bg-red-950/40 dark:text-red-200">
                            <x-icon :name="$feature['icon']" class="h-5 w-5" />
                        </span>
                        <p class="mt-5 text-sm font-semibold text-red-700 dark:text-red-300">{{ $feature['eyebrow'] }}</p>
    ...
    <section class="border-y border-zinc-200 bg-white py-16 dark:border-zinc-800 dark:bg-zinc-900/60">
        <div class="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[0.85fr_1.15fr] lg:px-8">
            <div>
                <p class="text-sm font-semibold uppercase text-red-700 dark:text-red-300">{{ __('How service stays organized') }}</p>
    ...
    <section class="bg-zinc-50 py-16 dark:bg-zinc-950">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
                <div class="max-w-2xl">
                    <p class="text-sm font-semibold uppercase text-red-700 dark:text-red-300">{{ __('Pricing') }}</p>
            ...
            <div class="mt-10 grid gap-4 lg:grid-cols-3">
                @foreach ($plans as $plan)
                    ...
                    <article @class([
                        ...
                        'border-red-300 ring-2 ring-red-700/10 dark:border-red-800 dark:ring-red-400/10' => $isFeatured,
                        ...
                        <div class="flex items-start justify-between gap-4">
                            ...
                            @if ($isFeatured)
                                <span class="rounded-full border border-red-200 bg-red-50 px-2.5 py-1 text-xs font-bold text-red-700 dark:border-red-900 dark:bg-red-950/50 dark:text-red-200">{{ __('Popular') }}</span>
                            @endif
                        </div>

                        <div class="mt-6 flex items-end gap-1">
                            <span class="text-4xl font-bold text-zinc-950 dark:text-white">${{ number_format(data_get($plan, 'monthly_price_cents', 0) / 100, 0) }}</span>
                        ...
                        <ul class="mt-6 space-y-3">
                            @foreach ($features as $feature)
                                <li class="flex gap-3 text-sm leading-6 text-zinc-700 dark:text-zinc-300">
                                    <x-icon name="check" class="mt-0.5 h-4 w-4 shrink-0 text-red-700 dark:text-red-300" />
                                    <span>{{ __($feature) }}</span>
                                </li>
                            @endforeach
                        </ul>

                        <a href="{{ route('restaurants.apply') }}" class="{{ $isFeatured ? 'bg-red-700 text-white hover:bg-red-800' : 'border border-zinc-300 text-zinc-800 hover:bg-zinc-100 dark:border-zinc-700 dark:text-zinc-100 dark:hover:bg-zinc-800' }} mt-7 inline-flex w-full items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-semibold transition app-focus">
                            {{ __('Start trial') }}
                            <x-icon name="arrow-right" class="h-4 w-4" />
                        </a>
                    </article>
