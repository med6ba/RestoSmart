<div x-data="notificationsDropdown()" class="relative">
    <button type="button" @click="toggle()" @click.outside="close()" class="relative rounded-full p-1.5 text-zinc-400 hover:text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 dark:hover:text-zinc-300 transition-colors">
        ...
        <span x-show="unreadCount > 0" x-transition class="absolute top-1 right-1 flex h-2.5 w-2.5">
    ...
    <div x-show="open" x-transition.opacity.duration.200ms style="display: none;" class="absolute right-0 z-50 mt-2 w-80 origin-top-right rounded-lg bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:bg-zinc-900 dark:ring-white/10">
