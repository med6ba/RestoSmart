    ...
    <div class="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div class="grid gap-4 lg:grid-cols-3">
            @forelse ($orders as $order)
                <article class="rounded-lg border bg-white p-4 {{ $order->type === 'delivery' ? 'border-red-300' : 'border-amber-300' }}">
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            <p class="text-sm font-semibold uppercase tracking-wide {{ $order->type === 'delivery' ? 'text-red-700' : 'text-amber-700' }}">{{ str_replace('_', ' ', $order->type) }}</p>
                    ...
                    <div class="mt-4 divide-y divide-stone-100">
                        @foreach ($order->items as $item)
                            <div class="py-2">
                                <p class="font-semibold">{{ $item->quantity }} × {{ $item->name }}</p>
                                @if ($item->notes)
