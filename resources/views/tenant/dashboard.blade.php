<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-1">
            <p class="text-sm font-semibold text-brand-700 dark:text-brand-300">{{ tenant('name') }}</p>
            <h1 class="text-xl font-semibold text-zinc-950 dark:text-white">{{ __('Client dashboard') }}</h1>
        </div>
    </x-slot>

    <div class="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8" data-realtime-scope="orders,menu">
        @if (session('status'))
            <div class="status-toast mb-6">{{ session('status') }}</div>
        @endif

        <section class="grid gap-4 md:grid-cols-2">
            <a href="{{ route('tenant.menu', tenant('id')) }}" class="rounded-lg border border-zinc-200 bg-white p-5 transition hover:border-brand-300 hover:bg-brand-50/40 app-focus dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-brand-900 dark:hover:bg-brand-950/20">
            ...
            <a href="{{ route('tenant.orders.index', tenant('id')) }}" class="rounded-lg border border-zinc-200 bg-white p-5 transition hover:border-brand-300 hover:bg-brand-50/40 app-focus dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-brand-900 dark:hover:bg-brand-950/20">
            ...
            <div class="rounded-lg border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <div class="mt-4 divide-y divide-zinc-100 dark:divide-zinc-800">
                    @forelse ($myOrders as $order)
                        <a href="{{ route('tenant.orders.show', [tenant('id'), $order]) }}" class="flex flex-col gap-2 py-3 transition hover:text-brand-700 app-focus sm:flex-row sm:items-center sm:justify-between dark:hover:text-brand-300">
                            <div>
                                <p class="font-semibold">{{ $order->public_code }}</p>
                                <p class="text-sm text-zinc-600 dark:text-zinc-300">{{ $order->typeLabel() }} · {{ $order->created_at->diffForHumans() }}</p>
                            </div>
                            <span class="w-fit rounded-full bg-zinc-100 px-3 py-1 text-xs font-semibold text-zinc-700 dark:bg-zinc-800 dark:text-zinc-200">{{ __(App\Models\Order::STATUS_FLOW[$order->status] ?? ucfirst($order->status)) }}</span>
                        </a>
                    @empty
